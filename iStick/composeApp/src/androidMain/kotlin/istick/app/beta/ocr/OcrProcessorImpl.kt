// File: iStick/composeApp/src/androidMain/kotlin/istick/app/beta/ocr/OcrProcessorImpl.kt
...
class AndroidOcrProcessor(
    ...
    override suspend fun extractMileage(imageBytes: ByteArray): Result<Int> = withContext(Dispatchers.Default) {
        try {
            ...
            Log.e(TAG, "Error extracting mileage: ${e.message}", e)
            when (e) {
    ...
    override fun extractMileageWithProgress(imageBytes: ByteArray): Flow<OcrProgress> = flow {
        try {
            ...
            Log.e(TAG, "Error in mileage extraction flow: ${e.message}", e)
            val message = when (e) {
    ...
    private fun extractMileageFromText(text: Text): Int? {
        ...
        Log.d(TAG, "Raw extracted text: ${text.text}")
        ...
        Log.d(TAG, "Extracted numbers: $allNumbers")
...
/**
 * Create OCR processor with Android context
 */
